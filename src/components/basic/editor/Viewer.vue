...
import './highlight/atelier-heath-light.css'
import './theme/fancy.scss'
...
const on = () => {
  if (props.plugins && file.value) {
    cbs.value = props.plugins.map(
      ({ viewerEffect }) => viewerEffect && viewerEffect({ markdownBody: instance?.subTree.el, file: file.value })
    )
  }
}
